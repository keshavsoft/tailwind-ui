...
const buildRow = ({ item, index, inVisibleColumnsConfig, searchValue,
    inShowSerial, inShowActions = false, onDeleteFunc
}) => {
    const tr = createRow({
    ...
    if (inShowActions) {
        appendActionCell({ tr, item, index, searchValue, onDeleteFunc });
    };
...
const appendActionCell = ({ tr, item, index, inShowActions, options = {},
    searchValue, onDeleteFunc }) => {
    // debugger;
